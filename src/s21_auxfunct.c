...
void CheckFloatTR(float src, int *mantissa_seven, int *scale_seven) {
  ...
  *scale_seven = 0;
  ...
  for (int i = 0, flag_dot = 0, flag_digit = 0, int_digit_found = 0, flag = 1;
       flag; i++) {
    ...
    delitel = (int_digit_found > 8) ? int_digit_found - 8
                                    : 0;  // чтобы получить 8ми значное число,
                                          // нужно src / (10 ^ delitel)
    flag = ((digit_found < 8 || flag_dot == 0) && *scale_seven < 28)
               ? flag
               : 0;  // выход из цикла: (поиск до 28 порядка) или (8 значащих
                     // цифр и найденной целой части числа)
  }
  *scale_seven = *scale_seven - delitel;
  if (digit_found >= 8) {
    // если цифр больше 8 округляем до 7ми банк округлением
    *mantissa_seven =
        (BankRoundTR((int)((double)src * pow(10, *scale_seven)))) / 10;
...
s21_decimal MantissaDiv10(s21_decimal numb) {
  ...
  for (int i = 95; i >= 0; i--) {
    ...
    int byte = i / 32;
    int bit = i % 32;
    if (numb.bits[byte] & (1UL << bit)) reminder |= 1;
    if (reminder >= 10) {
      reminder -= 10;
...
int getSign(s21_decimal numb) { return getDecimalBit(numb, 127); }
